...
import { directLabelPrintAsync, updatePrint } from '../store/printSlice';
...
const RecevingSchema = Yup.object().shape({
  generic_code: Yup.string().required('Required'), 
...
const ReceivingScreen = ({
  ...
  const _state = useSelector((state: RootState) => state);
  ...
  const hidePrintModal = () => {
    dispatch(updatePrint({ loading: false }))
    setPMVisible(false);
  };
  ...
  const [generic_code, setGenericCode] = useState('');
  ...
  const innventorySave = () => {
    ...
    const updatedValues = {
      ...
      person_supplier_id: personId,
      company_supplier_id: companyId,
      batch_number: values.purchaseCode,
      generic_code: values.generic_code,
      transfer_type_id: values.transferType,
      no_top_handling_unit: values.topQuantity,
      no_bottom_handling_unit: values.bottomQuantity,
      // type_id: values.itemType,
      subtype_id: values.itemSubType,
      category_id: values.itemCategory,
      subcategory_id: values.itemSubCategory,
      destination: locationId,
  ...
  useEffect(() => {
    ...
    const currentInventoryItem = itemInventoryList.find((item) => {
      console.log("value product =================" + values.productCode);
  ...
  // const handleEventChange = (event:any, key : string) => {
  //   const { name } = event.target;
  //   setGenericCode({...values.generic_code, [name]:value});
  //   setFormData({
  //     ...formData,
  //     [name]: value,
  //   });
    ...
    <Layout>
      <ScrollView nestedScrollEnabled={true}>
        <View style={styles.container}>
          <Card>
            <NormalInput
              fieldName={'generic_code'}
              navigation={navigation}
              placeholder="Purchase / Restock / Transfer Code"
              onChange={handleChange('generic_code')}
              handleId={setGenericCode}
              onBlur={handleBlur('generic_code')}
              value={values.generic_code}
              data={[]}
              style={{
                zIndex: 10,
              }}
              ...
              error={
                errors.generic_code && touched.generic_code
                  ? errors.generic_code
                  : ''
              }
            />
            ...
            <AutoCompleteNScanInput
              ...
              data={
                values.sourceType == 'person' ? peopleList : companyList
              }
              value={values.source}
              ...
              style={{zIndex: 9}}
              displayName={
                values.sourceType == 'person' ? 'user_name' : 'name'
              }
            ...
            <AutoCompleteNScanInput
              ...
              data={ positionList }
              style={{zIndex: 1}}
              error={ errors.destination && touched.destination ? errors.destination : '' }
            />
            ...
            <Modal onDismiss={hidePrintModal} isVisible={pmVisible}>
              <View style={styles.modalContainerStyle}>
                ...
                <AutoCompleteNScanInput
                  ...
                  style={{zIndex: 100}}
                  error={errors.embeddedDevice && touched.embeddedDevice ? errors.embeddedDevice : ''}
                />
                <View>
                  <Button
